...
- **Build cost visible while choosing.** Where candidate solutions vary by channel or vehicle (push versus email, in-app versus off-platform), name the rough build or infrastructure cost in each candidate as it is proposed, not after the choice. The cheap-to-build path should be visible during selection. A solution that needs a new surface built carries a different effort profile from one that reuses an existing channel, and that often decides the bet.
...
| **EVIDENCE-CHECK** | **Before a LOFA is carried to MVT design, ask whether evidence already bears on it.** As each LOFA is named, ask the user directly: is there a prior test, dataset, or analogue that already speaks to this? If yes, rate it from that evidence (cite per Phase 6c) instead of routing a fresh test; if the evidence is recalled but uncaptured, accept it at MEDIUM and log the citation gap. This stops the coach designing a full MVT for an assumption the team has already settled, which is easy to miss when the Phase 0.5 "what's been tried" probe under-surfaced it. | *"You flagged the demand-ceiling LOFA. Has anything tested this before? A prior trial lifted net new visits, so this reads as validated (MEDIUM, uncited), not a test target."* | Designing a full ceiling MVT, then learning a prior trial already validated it |
